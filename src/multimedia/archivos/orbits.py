...
class Orbit(Scene):
    def construct(self):
        orbitat1 = Text("Orbita:",font_size=36,weight=BOLD)
        orbitat2 = Text("Trayectoria circular de los planetas.",font_size=28).next_to(orbitat1, DOWN, buff=0.2)

        self.play(Write(orbitat1),Write(orbitat2))
        self.wait(1.5)
        self.play(FadeOut(orbitat1),FadeOut(orbitat2))

        jp = Dot(color=YELLOW)
        
        text = Tex(r"Cuando los planetas se alinean,\\",
                   r"la excentricidad orbital modifica la\\",
                   r"sincronizacion de sus encuentros." , font_size = 32).to_edge(UP)
